        ...
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                if parents.isEmpty {
                    ...
                    Text(parents.map { $0.displayName ?? $0.name }.joined(separator: ", "))
                        .font(.caption)
...
struct ParentManagementTab: View {
    ...
    var body: some View {
        ...
        .alert("Remove Parent?", isPresented: $showDeleteConfirmation) {
            ...
            if let parent = parentToDelete {
                Text("Are you sure you want to remove \(parent.displayName ?? parent.name)? They will no longer have access to the class.")
            }
        }
    }
...
struct ParentManagementRow: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            // Info
            VStack(alignment: .leading, spacing: 2) {
                Text(parent.displayName ?? parent.name)
                    .font(.headline)
            ...
            Form {
                ...
                Section("Link to Parent (Optional)") {
                    Picker("Parent", selection: $selectedParentId) {
                        ...
                        ForEach(classroomViewModel.parents) { parent in
                            Text(parent.displayName ?? parent.name).tag(parent.id as String?)
                        }
                    }
                }
            }
